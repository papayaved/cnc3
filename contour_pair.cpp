...
bool ContourPair::fixReverse(const fpoint_valid_t &bot_prev, const fpoint_valid_t &top_prev) {
    return !botEmpty() && m_bot->fixReverse(bot_prev) && (topEmpty() || m_top->fixReverse(top_prev));
}

bool ContourPair::checkSorted(const fpoint_valid_t& bot_prev, const fpoint_valid_t& top_prev) {
    return !botEmpty() && m_bot->checkSorted(bot_prev) && (topEmpty() || m_top->checkSorted(top_prev));
}
